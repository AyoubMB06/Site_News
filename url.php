...
session_start();
  ...
  $con = new MongoDB\Client("mongodb://localhost:27017");
    ...
    if(isset($_SESSION["session_login"])){
        ...
        $nomUser=$cursor['nom'];
        $prenomUser=$cursor['prenom'];



        $collection1 = $db->avis;
        $cursor = $collection1->findOne(array('titre' => $_GET['titre2']));
        
        $pub1['nom']=$cursor['nom'];
        $pub1['prenom']=$cursor['prenom'];
        $pub1['titre']=$cursor['titre'];
        $pub1['sujet']=$cursor['sujet'];
        $pub1['date']=$cursor['date'];
...
<header class="w3-container w3-center w3-padding-32"> 
  ...
  <p>Bienvenue à la page des news <span class="w3-tag"><?php  echo $nomUser; ?></span></p>
</header>
...
<?php    
        ...
        $avis = $db->avis;
        $comments  = $avis->findOne(array('titre'=>$pub1['titre']));
        $nb=$comments->count();
        $total=$nb-10; 
...
<div class="w3-col l8 s12" >
  ...
  <div class="w3-card-4 w3-margin w3-white">
    ...
    <div class="w3-container" >
      ...
      <h5><?php echo $comments['commentaire'.$i]['Nom']." ".$comments['commentaire'.$i]['Prenom'].", " ?><span class="w3-opacity"><?php echo $comments['commentaire'.$i]['date2'] ?></span></h5>
    </div>
 
    <div class="w3-container">
      <p><?php echo $comments['commentaire'.$i]['commentaire'] ?></p>
